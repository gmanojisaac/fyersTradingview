...
var accesstoken = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..f8dKdp1cph9JdytBnIhcMDMDKroBHKLbNyTUUcyTt2M";
...
app.post('/submit-form', (req, res) => {
    ...
    skt.on("message", function (message) {
        //console.log('Reached here',subscribeList);

        if (textreceived == 'HUNTING') {
            switch (message.symbol) {
                case 'NSE:NIFTYBANK-INDEX':
                    //console.log('reached here', message.ltp);
                    if (BTagsReceivedState === false) {
                        BNiftyCE = BGenericTag + (Number(message.ltp) - (Number(message.ltp) % 100)) + 'CE';
                        BNiftyPE = BGenericTag + (Number(message.ltp) - (Number(message.ltp) % 100) + 100) + 'PE';
            ...
            switch (message.symbol) {
                case 'NSE:NIFTYBANK-INDEX':
                    ...
                    break;
                case BNiftyCE:
                    //console.log('Reached here6', message.ltp);
